...
async function initTONConnect() {
  if (tonConnector) return tonConnector;
  
  // Load TON Connect UI from CDN
...
async function connectTONWallet() {
  try {
    ...
    showNotif('والِت TON با موفقیت متصل شد!', '💎');
    return true;
  } catch (e) {
    showNotif('اتصال والت ناموفق بود. دوباره امتحان کنید.', '⚠️');
    return false;
  }
}

async function sendTONPayment(amountTON, orderInfo) {
  try {
    const connector = await initTONConnect();
    ...
    const receiverAddress = "UQCpm9nRpvM3V7TGr5TUV8LKw2la-hYXKhP-txV_AjEW2mjq";
    
    // Simulate transaction (in real, use sendTransaction)
    showNotif(`در حال ارسال ${amountTON} TON به آدرس دریافت‌کننده...`, '🔄');
    
    setTimeout(() => {
      showNotif(`پرداخت ${amountTON} TON با موفقیت انجام شد! سفارش ثبت گردید.`, '✅');
    ...
    showNotif('خطا در پرداخت TON. لطفاً دوباره تلاش کنید.', '❌');
    return false;
  }
}

// Add to purchase confirmation
function confirmPurchaseWithTON() {
  const total = /* calculate from cart or modal */ 100; // replace with real total
  const tonAmount = (total / TON_RATE).toFixed(2);
  sendTONPayment(tonAmount, "خرید خودرو");
}
